...
#Ex-2
def buscarDuplicados(n,numbers):
    if (len(numbers)==1):
        return n==numbers[0]
    else:
        return buscarDuplicados(n,numbers[1:len(numbers)])
#Ex-2
...
def main():
    ...
    i=0
    dup = False
    while(not buscarDuplicados(numsE2[i],numsE2[1:len(numsE2)]) and i<len(numsE2)):
        i+=1
    if (i==len(numsE2)-1):
        print("Si hay duplicados")
    else:
